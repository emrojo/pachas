...
- **FR-35.8**: **Mobile-Dedicated Receipt Redaction View & Ergonomic Icon-Only Controls ([`ReceiptRedactionModal.tsx`])**:
  - **Identificación Automática en Dispositivos Móviles**: Detecta automáticamente el acceso desde smartphones (`isMobileView: true` o viewport responsivo `< 768px`) desde `/dashboard-mobile` y `ExpenseForm`.
  - **Aviso de Privacidad con Tipografía Legible**: Tarjeta de privacidad destacada (`bg-slate-900 border-slate-800`) con tipografía agrandada y legible (`text-sm sm:text-base font-bold text-amber-300` para título y `text-xs sm:text-sm text-slate-200 leading-relaxed font-normal` para cuerpo), garantizando lectura clara en pantallas pequeñas sin necesidad de zoom.
  - **Botonera Táctil Solo con Iconos Grandes**: Suprime etiquetas de texto para evitar saturación y saltos de línea irregulares, utilizando botones de 48x48px (`h-12`) con iconos nítidos de `w-6 h-6 stroke-[2.2]` para Pincel (`Paintbrush`), Caja (`Square`), Borrador (`Eraser`) y Mover/Pan (`Hand`).
  - **Selector de Grosor Visual por Puntos**: Reemplaza el texto ("Fino", "Medio", "Grueso") por puntos circulares concéntricos (fino: 8px, medio: 14px, grueso: 20px) en botones táctiles de 40x40px.
  - **Controles de Zoom, Deshacer y Limpiar**: Botones de 44x44px con iconos `w-5 h-5` para zoom y acciones de historial, con feedback háptico (`active:scale-95`).
  - **Botones de Acción Inferiores Ergonómicos**: Botones amplios y cómodos para el pulgar (*"Cancelar"* y *"Procesar con IA 🚀"*).
- **FR-35.9**: **Mobile-Dedicated Receipt Validation View & Ergonomic Form Layout ([`ReceiptValidationModal.tsx`])**:
  - **Identificación Automática en Dispositivos Móviles**: Detección automática al operar desde smartphones (`isMobileView: true` o `window.innerWidth < 768px`) al abrir tickets pendientes desde `/dashboard-mobile` o notificaciones.
  - **Aviso de Privacidad y Responsabilidad con Tipografía Legible**: Banner con tipografía contrastada y de fácil lectura en pantalla reducida (`text-xs sm:text-sm` para texto explicativo y `text-xs sm:text-sm uppercase tracking-wider` con icono `w-5 h-5` para título).
  - **Botonera de Censura Mini Solo con Iconos**: Herramientas de retoque de censura (Zoom, Marcador, Caja, Borrador, Deshacer) representadas exclusivamente por iconos grandes (`w-5 h-5`) con áreas táctiles mínimas de 44x44px (`w-11 h-11`), eliminando texto para evitar saturación y permitiendo interacción táctil cómoda.
  - **Importe Financiero Heroico**: Campo de importe principal destacado con tipografía monoespaciada de gran tamaño (`text-3xl font-mono font-black text-emerald-600`) acompañado de selector de divisa táctil (`py-3.5 text-base rounded-2xl`).
  - **Tarjeta de Auditoría Matemática y Desglose Táctil**: Indicador de consistencia de impuestos con badges táctiles para alternar IVA incluido/excluido, desglose de subtotales legible (`text-xs`) y botón de ajuste rápido en caso de discrepancias (`py-2 px-3.5`).
  - **Categorías Táctiles Amplias en Grid**: Cuadrícula de categorías con botones ergonómicos de 3 columnas (`py-3 px-2 rounded-2xl`), iconos emoji agrandados (`text-2xl`) y nombres directos.
  - **Acciones Inferiores Ergonómicas**: Botones táctiles de gran tamaño: botón de descarte (`Trash2`) y botón de cierre (`X`) como iconos de 48x48px (`w-12 h-12 rounded-2xl`), y botón primario de confirmación ancho y prominente (`py-3.5 px-4 rounded-2xl font-black text-sm sm:text-base`) para crear el gasto con un solo toque del pulgar.
...
- **FR-60.7**: **Mobile-Dedicated Itemized Split Controls & Informational Tax Breakdown ([`src/components/expenses/ItemizedSplitEditor.tsx`](file:///d:/Projects/pachas/src/components/expenses/ItemizedSplitEditor.tsx))**:
  - **Tratamiento del IVA Informativo y No Editable (`isTaxReadOnly: true`)**: En la vista de validación de tickets escaneados por IA (`ReceiptValidationModal`), la información del IVA por producto se presenta como texto descriptivo fijo e informativo (*"Desglose con IVA"*), desactivando botones de tasa para evitar modificaciones innecesarias y preservando los valores inferidos por la IA.
  - **Controles Táctiles de Consumidores (Consumers) Agrandados en Móvil**: Botones de miembros con altura táctil cómoda (mínimo 44px), avatares agrandados, nombres en tipografía destacada y steppers `+` / `-` de mayor tamaño (`w-7 h-7`) para repartir unidades fácilmente con el pulgar.
  - **Inputs y Steppers Agrandados**: Título de producto, precio y selector general de cantidad con mayor tamaño y contraste para pantallas táctiles reducidas.
  - **Comentarios y Textos Resumidos**: Indicadores de cabecera, tarjeta de balanceo y badges de asignación sintéticos y directos para optimizar el espacio vertical en pantallas móviles.
...
- **FR-74.9**: **Invoice-Wide Tax Uniformity & Per-Product Tax Inclusion Persistence (`public.expense_items.tax_included`)**:
  - Enforces the commercial invoice invariant: pricing policy is uniform across all line items on a receipt (if one product includes tax in its printed price, all do; if one excludes it, all do).
  - Automatically infers and propagates tax inclusion to 100% of line items in OCR and mathematical auditing.
  - Persists `tax_included BOOLEAN DEFAULT TRUE NOT NULL` per product in `public.expense_items` via database migration `21-expense-items-tax-included.sql` and REST API endpoints (`POST /api/expenses`, `PUT /api/expenses/[id]`).
  - Motor de cálculo `calculateItemizedSplits` respeta `tax_included` deduciendo cuotas de precios PVP o sumándolas a bases netas, previniendo la duplicación errónea de impuestos en repartos minoristas.
- **FR-74.10**: **Detección de Jurisdicción Europea, Factura Simplificada vs. Completa y Decremento Legal de IVA (`is_europe`, `invoice_type`, `net_price`)**:
  - **Identificación de Jurisdicción Europea**: Detecta si el establecimiento emisor pertenece a la Unión Europea (`is_europe: boolean`) a partir del código de país, moneda (EUR), terminología impositiva (IVA, TVA, MwSt, VAT) o geolocalización.
  - **Clasificación de Tipo de Factura**: Clasifica automáticamente la factura (`invoice_type: 'simplified' | 'full' | 'standard' | 'other'`) diferenciando facturas simplificadas (tickets ordinarios a consumidor final) de facturas completas/ordinarias con desglose nominal de cliente.
  - **Determinación de Legislación Fiscal**: Registra y persiste la legislación aplicable (`tax_legislation`), por ejemplo Directiva Europea del IVA 2006/112/CE (`EU_DIRECTIVE_2006_112`) o Reglamento de Facturación de España RD 1619/2012 (`ES_RD_1619_2012`).
  - **Regla Legal Europea de Precios con IVA y Decremento a Base Imponible**: En Europa, bajo el Art. 7.1.f del RD 1619/2012 y la Directiva 2006/112/CE, en las facturas simplificadas todos los precios mostrados incluyen obligatoriamente el IVA por ley (`tax_included = true` e `items_price_includes_tax = true`). El motor calcula de forma determinista el precio antes de impuestos (`net_price`) de cada producto decrementando su tipo impositivo correspondiente:
    $$\text{net\_price} = \text{round}_2\left(\frac{\text{price}}{1 + \text{rate}/100}\right), \quad \text{tax\_amount} = \text{round}_2(\text{price} - \text{net\_price})$$
  - **Cálculo de Base Imponible**: Si el subtotal/base imponible no viene impreso en el ticket, se calcula sumando las bases netas de todos los productos ($\sum \text{net\_price}$) o sustraendo la cuota global del total.
  - **Persistencia en Base de Datos**: Migración `22-invoice-type-and-legislation.sql` añade `invoice_type`, `tax_legislation`, `is_europe` a `public.expenses`, y `net_price NUMERIC(10,2)` a `public.expense_items`, con declaraciones de auto-reparación en `postgres.ts` y soporte de fallback transaccional resiliente en endpoints REST.
  - **Integración en Interfaz de Usuario**: Distintivo visual en `ItemizedSplitEditor` con acreditación europea (*"🇪🇺 Factura simplificada europea • IVA incluido por ley (RD 1619/2012)"*), actualización dinámica de `net_price` al modificar precios o tramos de IVA en caliente, y sincronización en formularios y modales de validación.
...
| **13/09/2026** | 🧾 Enhanced | **FR-74.5 - FR-74.9** | **Multi-Bracket Tax Reconciliation, Product Tax Inference & Invoice Uniformity Persistence**: Injected country-specific VAT brackets (Spain, Germany, France, etc.) and category guidelines into Gemini OCR prompt; differentiated item tax inclusion (`items_price_includes_tax`); enforced invoice uniformity invariant (tax inclusion applies uniformly across 100% of receipt products); persisted `tax_included` per product in `public.expense_items` (migration 21); adapted `calculateItemizedSplits` to prevent double-taxation on PVP prices; forwarded tax metadata and detection badges in `ExpenseForm.tsx` and `ItemizedSplitEditor.tsx`. |
| **13/09/2026** | 🇪🇺 Added | **FR-74.10** | **European Jurisdiction Detection, Simplified vs. Full Invoice Classification & Legal VAT Decrementing**: Detection of European establishments (`is_europe`), classification of invoice type (`invoice_type: 'simplified' | 'full' | 'standard' | 'other'`) and tax legislation (`tax_legislation: 'EU_DIRECTIVE_2006_112'`). Enforces European legal rule (Art. 7.1.f RD 1619/2012 / Directive 2006/112/EC) where simplified invoices mandate prices including VAT, deterministically decrementing VAT to calculate `net_price = round2(price / (1 + rate / 100))` and item `tax_amount`. Database migration `22-invoice-type-and-legislation.sql`, European compliance badges in UI, and full test suite. |
