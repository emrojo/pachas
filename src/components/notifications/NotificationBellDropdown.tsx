...
export const NotificationBellDropdown: React.FC = () => {
  ...
  const resolveNotificationUrl = (notif: (typeof notifications)[0]): string => {
  ...
  return (
    <div className="relative inline-block" ref={dropdownRef}>
      ...
      {/* Floating Dropdown */}
      {isOpen && (
        <div className="fixed sm:absolute left-3 right-3 sm:left-auto sm:right-0 top-16 sm:top-full mt-2 w-auto sm:w-96 max-w-[calc(100vw-1.5rem)] sm:max-w-none bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl shadow-2xl overflow-hidden z-50 animate-in fade-in zoom-in-95 duration-150 flex flex-col max-h-[calc(100vh-5.5rem)] sm:max-h-[500px]">
          {/* Header */}
          <div className="p-3.5 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between bg-slate-50/50 dark:bg-slate-900/50">
            ...
            {unreadNotificationsCount > 0 && (
              ...
              >
                ...
                <span>{t('notifications.markAllRead') || 'Marcar leídas'}</span>
              </button>
            )}
          </div>
          ...
          <div className="px-3 py-2 border-b border-slate-100 dark:border-slate-800 flex items-center gap-2">
            ...
            >
              {t('notifications.unread') || 'No leídas'} ({unreadNotificationsCount})
            </button>
          </div>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {notif.type === 'receipt_pending' && (
                      <span className="inline-flex items-center gap-1 mt-1.5 px-2 py-0.5 rounded-full text-[10px] font-bold bg-amber-100 dark:bg-amber-950 text-amber-800 dark:text-amber-300">
                        🧾 {t('notifications.validateScan') || 'Requiere validación'}
                      </span>
                    )}
                  </div>
          ...
          <div className="p-2 border-t border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50 space-y-1.5 text-center">
            ...
            <Link
              href="/notifications"
              onClick={() => setIsOpen(false)}
              className="inline-flex items-center justify-center gap-1 text-xs font-bold text-emerald-600 dark:text-emerald-400 hover:text-emerald-700 transition-colors w-full py-0.5"
            >
